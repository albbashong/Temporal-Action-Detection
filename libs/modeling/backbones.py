...
from .blocks import (get_sinusoid_encoding, TransformerBlock,ShortLongBlock,MaskedConv1D,
                     ConvBlock, LayerNorm)
...
class ConvTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        for idx in range(arch[1]):
            self.stem.append(ShortLongBlock(fpn_dim=n_embd,stride=1))

        # main branch using transformer with pooling
        self.branch = nn.ModuleList()
        for idx in range(arch[2]):
                self.branch.append(ShortLongBlock(fpn_dim=n_embd,stride=2))


        self.stem_info=nn.Conv1d(n_embd,n_embd,3,1,1)
        self.embd_info=nn.Conv1d(n_embd,n_embd,3,1,1)
