...
class ShortLongBlock(nn.Module):
    def __init__(self,fpn_dim,stride):
        ...
        self.max_norm=LayerNorm(fpn_dim)


        self.long_conv=nn.Conv1d(fpn_dim,int(fpn_dim/8),7,stride,3)
        self.short_conv=nn.Conv1d(fpn_dim,int(fpn_dim/8),3,stride,1)

        self.deconv=nn.Conv1d(int(fpn_dim/8),fpn_dim,1,1,0)
        self.concat_info_norm=LayerNorm(fpn_dim)
        for i in range(1):
    ...
    def forward(self,x,mask):
        ...
        concat_info = long + short
        concat_info = self.deconv(concat_info)
        concat_info = self.concat_info_norm(concat_info)

        out=(out+max_out)+ concat_info.mean(dim=-1, keepdim=True)
