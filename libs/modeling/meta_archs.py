...
class PtTransformerClsHead(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.act = act_layer()
        self.avg_pool = nn.AvgPool1d(1, 1, 0)
        ...
        self.max_pool=nn.MaxPool1d(3,2,1)
    ...
    def forward(self, fpn_feats, fpn_masks):
        ...
        for _, (cur_feat, cur_mask) in enumerate(zip(fpn_feats, fpn_masks)):
            cur_out = cur_feat
            if prev_feat != None:
                cur_out = cur_out + self.max_pool(prev_feat)
...
class PtTransformer(nn.Module):
    ...
    def forward(self, video_list):
        ...
        #info = self.info_block(fpn_feats,fpn_masks)
        ...
        if self.training:
            ...
            gt_train_labels.append(gt_labels[0][0])
            gt_train_labels.append(gt_labels[1][0])
    ...
    def losses(
        ...
        list_1=torch.logical_and(gt_cls[:out_offsets[0].size(0)].sum(-1)>0,valid_mask[:out_offsets[0].size(0)])
        index_list_1=torch.nonzero(list_1)
          # True index of gt_cls
        pos_mask = torch.logical_and((gt_cls.sum(-1) > 0), valid_mask)  # True index of positive samples
        ...
        #pred_offsets=F.relu(pred_offsets+err_offsets)
