...
class ANETdetection(object):
    ...
    def wrapper_compute_average_precision(self, preds):
        ...
        for tuples in results:
            short_range.append(tuples[1])
            middle_range.append(tuples[2])
            large_range.append(tuples[3])
            extra_large_range.append(tuples[4])
            ap_results.append(tuples[0])
        ...
        all_middle_length=0
        all_large_length=0
        all_extra_large_length=0



        for idx in range(len(short_range)):
            if len(np.bincount(short_range[idx].astype('int64')))>=2:
                short_value+=np.bincount(short_range[idx].astype('int64'))[1]
            if len(np.bincount(middle_range[idx].astype('int64')))>=2:
                middle_value +=np.bincount(middle_range[idx].astype('int64'))[1]
            if len(np.bincount(large_range[idx].astype('int64')))>=2:
                large_value +=np.bincount(large_range[idx].astype('int64'))[1]
            if len(np.bincount(extra_large_range[idx].astype('int64')))>=2:
                extra_large_value +=np.bincount(extra_large_range[idx].astype('int64'))[1]
            all_short_length += np.count_nonzero(short_range[idx])
            all_middle_length += np.count_nonzero(middle_range[idx])
            all_large_length += np.count_nonzero(large_range[idx])
            all_extra_large_length += np.count_nonzero(extra_large_range[idx])

        print("short: "+str(short_value/all_short_length) )
        print("middle: " + str(middle_value / all_middle_length))
        print("large : " + str(large_value / all_large_length))
        print("extra large: " + str(extra_large_value / all_extra_large_length))

        for i, cidx in enumerate(self.activity_index.values()):
            ap[:,cidx] = ap_results[i]
...
def compute_average_precision_detection(
    ...
    ranges = [(0, 5), (5, 30), (30, 60), (60, 5000)]
    range_short = np.zeros(len(prediction))
    range_middle = np.zeros(len(prediction))
    range_long = np.zeros(len(prediction))
    range_extra_long = np.zeros(len(prediction))
    ...
    for idx, this_pred in prediction.iterrows():
        ...
        try:
            ...
            ground_truth['duration'] = ground_truth['t-end'] - ground_truth['t-start']
            prediction['duration'] = prediction['t-end'] - prediction['t-start']
        ...
        for tidx, tiou_thr in enumerate(tiou_thresholds):
            ...
            if tidx==0 and fp[tidx, idx] == 1: ## thresholds iou 0.3
                r_idx = 0
                for r in ranges:
                    r = [int(x) for x in r]
                    this_gt = this_gt.reset_index(drop=True)
                    gt_range = this_gt[(this_gt['duration'] >= r[0]) & (this_gt['duration'] < r[1])]
                    gt_range = gt_range.iloc[:, 5]
                    if not gt_range.empty:
                        if r_idx==0:
                            range_short[idx] = 1
                        if r_idx == 1:
                            range_middle[idx] = 1
                        if r_idx == 2:
                            range_long[idx] = 1
                        if r_idx == 3:
                            range_extra_long[idx] = 1
                    r_idx+=1

            if tidx==0 and tp[tidx, idx] == 1: ## thresholds iou 0.3
                r_idx = 0
                for r in ranges:
                    r = [int(x) for x in r]
                    this_gt = this_gt.reset_index(drop=True)
    ...
    return ap ,range_short,range_middle,range_long,range_extra_long
